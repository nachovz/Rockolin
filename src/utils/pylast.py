...
class Country(_BaseObject):
    ...
    def get_metros(self):
        """Returns a list of available metros for this country. Note:
            if the country name is not valid, then all available metros
            are returned, as per the LastFM API."""
        doc = self._request('geo.getMetros', True)

        seq = []

        for node in doc.getElementsByTagName('metro'):
            name = _extract(node, 'name')
            country = _extract(node, 'country')

            # returning a list of Metro rather than TopItem
            # because we don't have weights
            seq.append(Metro(name, country, self.network))
...
class Metro(_BaseObject):
    ...
    def __repr__(self):
        return "pylast.Metro(%s, %s, %s)" %(repr(self.name),
                repr(self.country), repr(self.network))
    ...
    def __ne__(self, other):
        return self.get_name() != other.get_name() or \
                self.get_country() != other.get_country()
    
    def _get_params(self):
        return {'country': self.get_country(), 'metro': self.get_name()}
    ...
    def get_top_tracks(self):
        return self._get_top_specific_tracks('')

    def get_hype_tracks(self):
        return self._get_top_specific_tracks('Hype')

    def get_unique_tracks(self):
        return self._get_top_specific_tracks('Unique')

    def _get_top_specific_tracks(self, track_type):
        doc = self._request('geo.getMetro' + track_type + 'TrackChart', True)
    ...
    def get_top_artists(self):
        """Returns a sequence of the most played artists for this metro."""
        return self._get_top_specific_artists('')

    def get_hype_artists(self):
        return self._get_top_specific_artists('Hype')

    def get_unique_artists(self):
        return self._get_top_specific_artists('Unique')
    
    def _get_top_specific_artists(self, artist_type):
        doc = self._request('geo.getMetro' + artist_type + 'ArtistChart', True)
